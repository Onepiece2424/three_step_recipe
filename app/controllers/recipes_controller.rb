class RecipesController < ApplicationController
  ...
  def index
    @recipes = Recipe.with_attached_images.order(:updated_at=>:desc,:created_at=>:desc).limit(5)
    @my_recipes = current_user.recipes.order(:updated_at=>:desc,:created_at=>:desc).limit(5)
  end
